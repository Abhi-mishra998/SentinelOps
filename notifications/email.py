...
class EmailNotifier:
    def __init__(self):
        ...
        self.from_addr = os.environ.get("ALERT_FROM_EMAIL", "sre-agent@yourdomain.com")
        self.to_addrs  = os.environ.get("ALERT_TO_EMAILS", "").split(",")

    def send_incident_detected(self, incident: dict):
        subject = f"[SRE Agent] Incident Detected: {incident.get('pod_name')} ({incident.get('incident_type')})"
        body = self._html_body(incident)
        self._send(subject, body)

    def send_incident_resolved(self, incident: dict):
